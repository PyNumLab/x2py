...
class FCodePrinter(CodePrinter):
    ...
    def _print_Import(self, expr):
        ...
        if len(targets) == 0:
            if isinstance(expr.source_module, FunctionDef) and expr.source_module.is_external:
                if expr.source_module.results:
                    ...
                    dtype = self._print(out_args[0].dtype.primitive_type) + ', '
                else:
                    dtype = ''
                return '{}external :: {}\n'.format(dtype, source)
        ...
        for i in targets:
            ...
            if isinstance(new_name, str):
                ...
                raise TypeError(
                    "Expecting str, PyccelSymbol or AsName, "
                    "given {}".format(type(i))
                )
    ...
    def _print_PyccelArrayShapeElement(self, expr):
        ...
        if isinstance(arg.class_type, NumpyNDArrayType):
            ...
            if arg.order == "C":
                index = PyccelMinus(LiteralInteger(arg.rank), expr.index)
        ...
        if isinstance(expr_type, CustomDataType):
            ...
            if rank > 0:
                ...
                if intent_in:
                    ...
                    ubounds = [
                        PyccelMinus(s, LiteralInteger(1))
    ...
    def _print_Allocate(self, expr):
        ...
        if isinstance(
            ...
            shape_code = ", ".join(
                "0:" + self._print(PyccelMinus(i, LiteralInteger(1)))
                for i in shape
            )
            if shape:
