...
test-enforced matrix lists every collected wrapper node as `legacy`,
`dual-route`, `wrapper-plan`, `not-applicable`, or `deferred-real-library` and
records the zero-legacy completion target.
