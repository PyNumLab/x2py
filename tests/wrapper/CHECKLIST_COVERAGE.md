...
| Shared native-array runtime handles expose common metadata, state dispatch, completed `to_numpy()` policy dispatch for borrowed views, read-only detached copies, and strided descriptor views, policy-gated pointer operations, and clear unavailable-operation errors for missing descriptor extraction | `tests/test_runtime_handles.py` |
...
| Native `Addr(Arg(...))` projection remains primitive-scalar-only while array descriptor handles use native-array handle policy blockers until descriptor handoff exists | `tests/semantics/test_ownership_policy.py::test_policy_completion_rejects_addr_projection_for_array_descriptor_handles`, `tests/semantics/test_ownership_policy.py::test_native_array_handle_policies_complete_before_ir_lowering`, `derived_types/test_pointers.py::test_pointer_array_handles_block_until_descriptor_handoff_exists` |
