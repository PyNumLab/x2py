...
def test_dense_strided_and_projected_arrays_match_legacy_and_wrapper_plan_routes(tmp_path: Path):
    ...
    for route, route_kwargs in (
        ...
        contract_package = tmp_path / f"{route}_multid_arrays"
        ...
        modules[route] = module if hasattr(module, "scale2_contiguous") else _sole_native_module(module)
    ...
    with pytest.raises(TypeError):
        direct.scale2_strided(_reversed_fortran_matrix(), output)
    with pytest.raises(TypeError):
        direct.scale2_strided(_broadcast_fortran_like_matrix(), output)
    with pytest.raises(TypeError):
        direct.scale2_contiguous(np.array(dense, order="C"), output)
