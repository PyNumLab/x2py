...
CALLBACK_SCALAR_F90_SOURCE = wrapper_source("fcallback_scalar_f90.f90")
...
def test_immediate_scalar_dummy_procedure_calls_python_callback(pyi_parity_build_mode: str, tmp_path: Path):
    ...
    if pyi_parity_build_mode == "source":
        wrapper_source = (
            _callback_scalar_build_dir(tmp_path, pyi_parity_build_mode) / "fcallback_scalar_f90_wrapper.c"
...
def test_callback_exception_prints_traceback_and_aborts_host_process(
    ...
    build_dir = _callback_scalar_build_dir(tmp_path, pyi_parity_build_mode)
    script = """
import numpy as np
