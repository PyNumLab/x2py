...
import x2py.extensions as ext_folder
...
# get path to x2py/stdlib/lib_name
stdlib_path = Path(stdlib_folder.__file__).parent

# get path to x2py/extensions_install/lib_name
...
class StdlibInstaller:
    ...
    **kwargs : dict
    ...
    def install_to(self, x2py_dirpath, installed_libs, verbose, compiler):
        """
        Install the files to the X2py dirpath.

        Install the files to the X2py dirpath so they can be easily located and analysed by
        ...
        x2py_dirpath : str | Path
            The path to the X2py working directory where the copy should be created.
        ...
        CompileObj
            ...
            library.
        """
        lib_dest_path = x2py_dirpath / self._folder
...
class ExternalLibInstaller:
    """
    A class describing how external libraries used by X2py are installed.

    A class describing how external libraries used by X2py are installed. An Installer
    ...
    dest_dir : str
        ...
        decides the name of the folder that will be created in the `__x2py__` folder.
...
class STCInstaller(ExternalLibInstaller):
    ...
    def install_to(
        ...
        use_pkg_config : bool, default=True
            Indicates if pkg-config should be used to locate STC before checking for a X2py
            installation.
...
class GFTLInstaller(ExternalLibInstaller):
    ...
    def install_to(self, x2py_dirpath, installed_libs, verbose, compiler):
        ...
        Install the files to the X2py dirpath so they can be easily located and analysed by
        users. This function creates a symlink to the X2py folder containing the code as
