...
class Scope:
    ...
    def collect_all_type_vars(self):
        ...
        type_vars = {
            ...
            if type(t).__name__ == "TypingTypeVar"
        }
        if self.parent_scope:
    ...
    def collect_tuple_element(self, tuple_elem):
        ...
        if (
            ...
            and isinstance(tuple_elem.base.class_type, BindCArrayType)
        ):
            for element, alias in self.symbolic_aliases.items():
                if (
                    ...
                    return alias
            raise RuntimeError(
                f"Bind-C array element {tuple_elem} has no symbolic alias"
            )
    ...
    def collect_all_tuple_elements(self, tuple_var):
        ...
        if isinstance(tuple_var, Variable) and isinstance(
            tuple_var.class_type, BindCArrayType
        ):
            return [
                self.collect_tuple_element(IndexedElement(tuple_var, i))
