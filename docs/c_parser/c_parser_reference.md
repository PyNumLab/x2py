...
array, function, and parenthesized combinations. The x2py CLI exposes shared
C/Fortran preprocessing flags and can run an exact C compiler/preprocessor
command before parsing compiler stdout, with `#line`/linemarker remapping for
parsed declarations and diagnostics.
