...
6. User-facing source-driven examples show the complete input source file before
   ...
   a clearly established local copy of that path, such as the basename named in
   the preceding prose. Generated-path command examples must use a path produced
   by an immediately preceding command. Placeholder filenames may appear as
   shorthand only after a concrete input-first example has established the
   workflow. Each command example should also show the expected result: stdout
   for inspection commands, generated contract text for `.pyi` commands, or a
   representative artifact tree for build commands.
